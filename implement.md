...
- `scripts/bootstrap/*` — state용 Storage 등 **최초 수동·CLI 친화** 부트스트랩
  - **환경별 설정:** `scripts/bootstrap/env/bootstrap-state.{dev,stg,prod}.env.example`를 복사해 같은 디렉터리에 `bootstrap-state.<env>.env`로 두고 이름(특히 스토리지 계정 전역 유일)을 맞춘다. 실사용 `.env`는 gitignore된다.
  - **생성:** `make bootstrap-state-dev` / `bootstrap-state-stg` / `bootstrap-state-prod`, 또는 `BOOTSTRAP_ENV=stg make bootstrap-state`. 내부적으로 `bootstrap-state-storage.sh --env-file ...`를 호출한다.
  - **직접 실행:** `./scripts/bootstrap/bootstrap-state-storage.sh --env-file scripts/bootstrap/env/bootstrap-state.dev.env` 등.
  - **되돌리기(파괴적):** 부트스트랩 전용 RG 전체를 지운다 — 원격 `.tfstate`까지 사라진다. `./scripts/bootstrap/bootstrap-state-teardown.sh --yes --env-file ...` 또는 `BOOTSTRAP_TEARDOWN_YES=1 make bootstrap-state-teardown-dev`(stg/prod 동일). RG 안에 다른 리소스가 있으면 함께 삭제된다.
