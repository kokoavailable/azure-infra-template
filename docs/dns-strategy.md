...
| Private DNS       | **Environment hub-centric** — private DNS zones that serve Azure PaaS private endpoints and internal names are anchored from each environment hub design; spokes **link** to zones and consume resolution accordingly. |
...
This aligns with `adr/0004-private-dns-centralization.md` and
`adr/0007-environment-owned-hub-and-spoke.md`.

## Design Principles

- Separate public and private DNS ownership.
- Keep internet-facing records under shared platform governance.
- Centralize private endpoint namespaces inside each environment hub to avoid duplicate zones and conflicting records inside that environment.
...
- Manage private zones from the **environment hub connectivity** perspective:
  zone lifecycle, linkage rules, and which VNets may participate.
