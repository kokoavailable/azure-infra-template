...
- Add or update a troubleshooting note when the incident revealed a reusable diagnostic or recovery pattern
- Create or update an ADR if the incident changes a long-term architecture decision

## Troubleshooting Notes

Use `docs/troubleshooting/` for durable notes about recurring project problems,
failed commands, confusing workflows, and recovery procedures.

Create a note when:

- The same failure is likely to happen again
- A command fails for a non-obvious reason
- Recovery required multiple steps
- The fix depends on repository-specific context
- Future Codex sessions or operators would benefit from a known-good diagnostic path

Do not record secrets, credentials, tenant IDs, subscription IDs, real backend
