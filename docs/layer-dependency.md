...
## Repository apply order (platform → environment stacks)
...
3. **Shared services**: `platform/shared-services/nonprod/` and/or `platform/shared-services/prod/` depending on scope.
4. **Environment hub**: `stacks/<env>/<region-code>/<azure-region>/hub/` — environment-owned hub network, private DNS posture, egress/security, and shared access paths.
5. **Environment spokes**: `stacks/<env>/<region-code>/<azure-region>/spokes/<spoke-name>/` — see canonical layer numbers in `stack-conventions.md`.

Cross-environment rule of thumb: establish or update **patterns** in dev first
when the change affects shared modules, addressing, or hub routing. Do not make
dev, staging, and production hub changes together unless explicitly required.
...
## Canonical Order (inside one environment)
...
Directory prefixes used in this repo map roughly as: `00-*`
foundation/network, `05-*` secrets or private DNS depending on hub/spoke
context, `06-*` configuration, `10-*` edge or egress depending on hub/spoke
context, `20-*` data or access operations depending on hub/spoke context,
`25-*` utility access, `30-*` compute, `40-*` observability.
...
**Note:** Egress, hub firewall routing, and shared ops ingress patterns are
environment hub concerns; see `architecture.md` and `security-baseline.md`.
...
Owns operator-facing access helpers that are not application runtime, such as:

- Dev-only utility or administrative virtual machines
- Jump or troubleshooting hosts when a hub-owned access path is not yet available
- Minimal access wiring required for private diagnostics or one-off operational tasks

Utility access is intentionally separate from runtime compute. Mutable utility hosts must not become hidden application dependencies, and application runtime stacks must not depend on utility VM state.
...
- Shared platform and environment hub services should be consumed through stable
  outputs, not by rediscovering resources through naming conventions alone.
...
- Apply shared platform stacks before environment stacks.
- Apply each environment hub before that environment's spokes.
